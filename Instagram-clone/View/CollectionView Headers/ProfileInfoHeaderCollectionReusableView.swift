...
final class ProfileInfoHeaderCollectionReusableView: UICollectionReusableView {
    ...
    private func addButtonActions() {
        followersButtons.addTarget(self,
                                   action: #selector(didTapFollowersButton),
                                   for: .touchUpInside)
        
        followingButtons.addTarget(self,
                                   action: #selector(didTapFollowingButton),
                                   for: .touchUpInside)
        
        postsButtons.addTarget(self,
    ...
    override func layoutSubviews() {
        ...
        postsButtons.frame = CGRect(x: profilePhotoImageView.right,
                                    y: 5,
                                    width: countButtonWidth,
                                    height: buttonHeight).integral
        
        followersButtons.frame = CGRect(x: postsButtons.right,
                                        y: 5,
                                        width: countButtonWidth,
                                        height: buttonHeight).integral
        
        followingButtons.frame = CGRect(x: followersButtons.right,
                                        y: 5,
                                        width: countButtonWidth,
                                        height: buttonHeight).integral
